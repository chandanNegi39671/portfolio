      ...
      <div className="relative p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-baseline gap-3 sm:gap-4">
            <span className={`font-display text-3xl font-bold ${accent.text} sm:text-4xl lg:text-5xl`}>
              {String(index + 1).padStart(2, "0")}
            </span>
            <div>
              <p className={`font-mono text-[10px] uppercase tracking-wider ${accent.text} sm:text-xs`}>
                {project.tag}
              </p>
              <h3 className="mt-1 font-display text-xl font-bold tracking-tight sm:text-3xl lg:text-4xl">
                {project.name}
              </h3>
            </div>
          </div>
        ...
        <div className="mt-8 grid grid-cols-3 gap-3 border-y border-line/60 py-6 sm:gap-4">
          {project.metrics.map((m) => (
            <div key={m.label}>
              <p className="font-display text-base text-ink sm:text-xl lg:text-2xl">{m.value}</p>
              <p className="mt-1 font-mono text-[10px] uppercase tracking-wider text-ink-faint sm:text-[11px]">
                {m.label}
              </p>
            </div>
          ))}
